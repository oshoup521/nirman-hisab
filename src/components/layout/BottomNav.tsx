...
import { LayoutDashboard, Construction, Hammer, Home, Settings } from 'lucide-react';
...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-100 px-2 pt-2 pb-[max(10px,env(safe-area-inset-bottom))] flex justify-around items-end z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.06)]">
      {tabs.map(({ id, label, Icon, color, pill }) => {
        ...
        return (
          <button
            ...
            className="flex flex-col items-center gap-0.5 flex-1 active:scale-95 transition-transform"
          >
            <div className={cn(
              'w-11 h-8 flex items-center justify-center rounded-xl transition-all duration-200',
