  ...
  return (
    <div className="space-y-3">
      ...
      <div className={cn(
        ...
        <p className={cn("text-[10px] font-bold uppercase tracking-wide", isProfit ? "text-emerald-100 dark:text-emerald-400" : "text-red-100 dark:text-red-400")}>Net Bachat</p>
        <p className={cn("text-[10px] mt-0.5", isProfit ? "text-emerald-200 dark:text-emerald-500/80" : "text-red-200 dark:text-red-500/80")}>Kamai − Kharcha (Paid)</p>
        <p className={cn("text-4xl font-bold mt-2 leading-none", isProfit ? "text-surface dark:text-emerald-300" : "text-surface dark:text-red-300")}>
          {isProfit ? '' : '−'}{formatCurrency(Math.abs(net))}
        </p>
        <p className={cn('text-xs font-bold mt-2', isProfit ? 'text-emerald-100 dark:text-emerald-400/80' : 'text-red-200 dark:text-red-400/80')}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-red-500/10 rounded-2xl p-4 border border-red-500/20">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <p className="text-[10px] font-bold text-red-500 uppercase tracking-wide">Kharcha</p>
          </div>
          <p className="text-xl font-bold text-red-600 dark:text-red-400 leading-none">{formatCurrency(totalKharcha)}</p>
        ...
        <div className="bg-emerald-500/10 rounded-2xl p-4 border border-emerald-500/20">
          ...
          <p className="text-xl font-bold text-emerald-600 dark:text-emerald-400 leading-none">{formatCurrency(totalRecovery)}</p>
        </div>
      </div>
      ...
      {totalBricks > 0 && (
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold text-amber-600 dark:text-amber-500 uppercase tracking-wide">Eetein Bachai</p>
              <p className="text-3xl font-bold text-amber-700 dark:text-amber-400 leading-none mt-0.5">
                {formatNumber(totalBricks)}
                <span className="text-sm font-bold text-amber-600 dark:text-amber-500 ml-1">pcs</span>
              </p>
              {brokenBricks > 0 && (
                <p className="text-[10px] text-text-subdued font-bold mt-1">{formatNumber(brokenBricks)} tooti hui</p>
              )}
            </div>
          </div>
          <div className="text-right">
            <p className="text-[10px] text-text-subdued font-bold uppercase">Value</p>
            <p className="font-bold text-emerald-600 dark:text-emerald-400 text-lg mt-0.5">{formatCurrency(brickRecoveryValue)}</p>
          </div>
        </div>
      )}
          ...
          {malwaCost > 0 && (
            <div className="flex justify-between items-center px-4 py-3 border-b border-border-subdued">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-sm text-text-secondary">Malwa Disposal</span>
              </div>
          ...
          {demolitionThekaCost > 0 && (
            <div className="flex justify-between items-center px-4 py-3 border-b border-border-subdued">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-sm text-text-secondary">Theka Diya (Paid)</span>
              </div>
          ...
          {demolitionThekaPending > 0 && (
            <div className="flex justify-between items-center px-4 py-3 border-b border-border-subdued">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-sm text-text-secondary">Theka Baaki (Pending)</span>
              </div>
              <span className="font-bold text-amber-500 text-sm">−{formatCurrency(demolitionThekaPending)}</span>
            </div>
          )}
          {scrapIncome > 0 && (
            <div className="flex justify-between items-center px-4 py-3 border-b border-border-subdued">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-sm text-text-secondary">Scrap / Kabaad</span>
              </div>
              <span className="font-bold text-emerald-600 dark:text-emerald-400 text-sm">+{formatCurrency(scrapIncome)}</span>
            </div>
          )}
          {brickRecoveryValue > 0 && (
            <div className="flex justify-between items-center px-4 py-3">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-sm text-text-secondary">Eent Recovery Value</span>
              </div>
              <span className="font-bold text-emerald-600 dark:text-emerald-400 text-sm">+{formatCurrency(brickRecoveryValue)}</span>
            </div>
          )}
        </div>
      )}
      ...
      {(state.demolitionThekas || []).length > 0 && (
        <div className="space-y-2.5">
          <p className="text-[10px] font-bold text-text-subdued uppercase tracking-wide px-1">Thekedar Status</p>
              ...
              <div key={t.id} className="bg-surface rounded-2xl border border-border-default shadow-sm p-4">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <span className="text-[10px] font-bold px-2 py-0.5 bg-amber-500/10 text-amber-500 rounded-full inline-block mt-1">
                      {t.workType}
                    </span>
                  </div>
                  <div className="text-right">
                    <p className={cn('font-bold text-sm', done ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500')}>
                      {done ? '✓ Full paid' : `−${formatCurrency(pending)}`}
                    </p>
                    <p className="text-[10px] text-text-subdued mt-0.5">{done ? 'Complete' : 'Baaki hai'}</p>
                  </div>
                </div>
                ...
                <div className="flex justify-between mt-1.5">
                  <p className="text-[10px] text-text-subdued">Diya: {formatCurrency(paid)}</p>
                  <p className="text-[10px] text-text-subdued">{pct.toFixed(0)}% complete</p>
                </div>
              </div>
            );
          })}
