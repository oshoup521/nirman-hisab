...
export default function TimelineSection() {
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-heading text-title font-bold text-text-primary">Kaam ki Raftaar</h3>
          ...
          <div key={milestone.id} className="relative">
            ...
            <div className="bg-surface p-4 rounded-2xl border border-border-default shadow-sm">
              <div className="flex justify-between items-center">
                <h4 className="font-heading text-title font-bold text-text-primary">{milestone.phase}</h4>
                <select
              ...
              {milestone.startDate && (() => {
                if (milestone.status === 'completed' && milestone.endDate) {
                  ...
                  return days >= 0 ? <p className="mt-2 text-caption text-text-subdued">{days} din lage</p> : null;
                }
                if (milestone.status === 'in-progress') {
                  const days = Math.round((Date.now() - new Date(milestone.startDate).getTime()) / 86400000);
                  return <p className="mt-2 text-caption text-brand font-medium">{days} din se chal raha hai</p>;
                }
                return null;
              })()}
