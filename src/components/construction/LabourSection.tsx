  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-heading text-title font-bold text-text-primary">Mazdoor & Theka</h3>
          <p className="text-caption text-text-subdued mt-0.5">{state.labours.length} workers</p>
        </div>
        <button
          onClick={openAdd}
      ...
      {state.labours.length === 0 ? (
        <div className="bg-surface rounded-2xl border border-border-default p-10 text-center">
          ...
          <p className="font-bold text-text-secondary text-body-sm">Koi mazdoor nahi abhi tak</p>
            ...
            return (
              <div key={labour.id} className="bg-surface rounded-2xl border border-border-default shadow-sm p-4 space-y-3">
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <h4 className="font-heading text-title font-bold text-text-primary">{labour.type}</h4>
                    <p className="text-caption text-text-subdued mt-0.5">{formatCurrency(labour.dailyWage)}/day</p>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-emerald-500/10 rounded-xl p-2.5 text-center">
                    <p className="text-title-lg font-bold text-emerald-600 dark:text-emerald-400 leading-none">{fullDays}</p>
                    <p className="text-caption text-emerald-600 dark:text-emerald-500 font-bold uppercase mt-1">Full Days</p>
                  </div>
                  <div className="bg-amber-500/10 rounded-xl p-2.5 text-center">
                    <p className="text-title-lg font-bold text-amber-500 leading-none">{halfDays}</p>
                    <p className="text-caption text-amber-500 font-bold uppercase mt-1">Half Days</p>
                  </div>
                  <div className="bg-brand/10 rounded-xl p-2.5 text-center">
                    <p className="text-title font-bold text-brand leading-tight">{formatCurrency(wages)}</p>
                    <p className="text-caption text-brand font-bold uppercase mt-1">This Month</p>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={cn(
                      'flex-1 py-2.5 rounded-xl text-body-sm font-bold border flex items-center justify-center gap-1.5 transition-all',
                      todayStatus === 'present'
          ...
          >
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <h3 className="font-heading text-title font-bold text-text-primary">{editId ? 'Mazdoor Edit' : 'Naya Mazdoor'}</h3>
              ...
              <div>
                <label className="text-caption font-bold text-text-subdued uppercase block mb-1.5">Labour Type</label>
              ...
              <div>
                <label className="text-caption font-bold text-text-subdued uppercase block mb-1.5">Daily Wage (₹)</label>
