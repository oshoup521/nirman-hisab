...
export default function OverviewSection() {
  ...
  if (!state.project) {
    return (
      <div className="bg-surface p-8 rounded-3xl border-2 border-dashed border-border-default text-center space-y-4">
        ...
        <div>
          <h3 className="font-bold text-text-primary">No Project Found</h3>
          <p className="text-text-subdued text-sm">Settings mein jaake project setup karein.</p>
        </div>
        <button
          onClick={() => setActiveTab('settings')}
          className="px-6 py-2 bg-brand text-surface rounded-full font-bold text-sm hover:opacity-90 transition-opacity"
        >
          Setup Project
        </button>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-surface p-5 rounded-3xl border border-border-default shadow-sm space-y-3">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-text-subdued text-sm">{project.location}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 pt-3 border-t border-border-subdued">
          <div>
            <p className="text-[10px] font-bold text-text-subdued uppercase mb-0.5">Shuru</p>
            <p className="font-bold text-text-primary text-sm">{format(new Date(project.startDate), 'dd MMM yyyy')}</p>
          </div>
          <div>
            <p className="text-[10px] font-bold text-text-subdued uppercase mb-0.5">Khatam (Plan)</p>
            <p className="font-bold text-text-primary text-sm">{format(new Date(project.endDate), 'dd MMM yyyy')}</p>
          </div>
        </div>
        <div className="pt-3 border-t border-border-subdued space-y-2">
          <div className="flex justify-between items-center">
            <p className="text-[10px] font-bold text-text-subdued uppercase">Time Progress</p>
            <p className={cn('text-[10px] font-bold', isOverdue ? 'text-red-500' : isAhead ? 'text-emerald-600 dark:text-emerald-400' : 'text-orange-500')}>
          ...
          <div className="flex justify-between text-[10px] text-text-subdued font-medium">
      ...
      <div className="bg-surface p-5 rounded-3xl border border-border-default shadow-sm space-y-3">
        <div className="flex justify-between items-center">
          <h4 className="font-bold text-text-primary">Kaam ki Progress</h4>
          <span className="text-sm font-bold text-brand">{phasePct}%</span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 pt-1">
          <div className="text-center">
            ...
            <p className="text-[10px] text-text-subdued font-bold uppercase">Done</p>
          </div>
          <div className="text-center">
            <p className="text-lg font-bold text-brand">{inProgress}</p>
            <p className="text-[10px] text-text-subdued font-bold uppercase">Chal Raha</p>
          </div>
          <div className="text-center">
            <p className="text-lg font-bold text-text-subdued">{pending}</p>
            <p className="text-[10px] text-text-subdued font-bold uppercase">Baaki</p>
          </div>
        </div>
        <button
          onClick={() => setSubTab('timeline')}
          className="w-full text-xs font-bold text-brand pt-2 border-t border-border-subdued hover:opacity-80 transition-opacity"
        >
          Sab phases dekho →
        </button>
      </div>
      ...
      {/* Kharcha Summary */}
      <div className="bg-surface p-5 rounded-3xl border border-border-default shadow-sm space-y-3">
        <h4 className="font-bold text-text-primary">Kharcha</h4>
        <div className="space-y-2">
          {[
            ...
            <div key={row.label} className="flex justify-between items-center text-sm">
              ...
              <span className="font-bold text-text-primary">{formatCurrency(row.value)}</span>
            </div>
          ))}
          <div className="flex justify-between items-center text-sm pt-2 border-t border-border-default">
            <span className="font-bold text-text-primary">Total</span>
            <span className="font-bold text-red-600 dark:text-red-400">{formatCurrency(totalSpent + totalMisc)}</span>
          </div>
        </div>
        <button onClick={() => setSubTab('expenses')} className="w-full text-xs font-bold text-brand pt-2 border-t border-border-subdued hover:opacity-80 transition-opacity">
          Detail dekho →
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <p className="text-xl font-bold text-text-primary">{materials.length}</p>
          <p className="text-[10px] font-bold text-text-subdued uppercase">Materials</p>
        ...
        >
          ...
          <p className="text-xl font-bold text-text-primary">{labours.length}</p>
          <p className="text-[10px] font-bold text-text-subdued uppercase">Mazdoor</p>
        </button>
      </div>
    </div>
  );
