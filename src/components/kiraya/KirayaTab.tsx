    ...
    <div className="space-y-5 pb-28">
      ...
      {rentals.length > 0 && (
        <div className="bg-brand dark:bg-brand-subdued dark:border dark:border-brand/20 rounded-3xl p-5 shadow-sm shadow-brand/20">
          <p className="text-surface dark:text-brand-text/70 text-caption font-bold uppercase tracking-wide mb-3">Monthly Overview</p>
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-black/15 dark:bg-white/10 rounded-xl px-3 py-2">
              <p className="text-surface/80 dark:text-brand-text/80 text-caption font-bold uppercase">Monthly Rent</p>
              <p className="text-surface dark:text-brand-text font-bold text-title-lg leading-tight">{formatCurrency(totalMonthlyRent)}</p>
            </div>
            <div className="bg-black/15 dark:bg-white/10 rounded-xl px-3 py-2">
              <p className="text-surface/80 dark:text-brand-text/80 text-caption font-bold uppercase">Rent Diya</p>
              <p className="text-surface dark:text-brand-text font-bold text-title-lg leading-tight">{formatCurrency(totalPaidRent)}</p>
            </div>
            {depositPaid > 0 && (
              <div className="bg-black/15 dark:bg-white/10 rounded-xl px-3 py-2">
                <p className="text-surface/80 dark:text-brand-text/80 text-caption font-bold uppercase">Deposit Diya ✓</p>
                <p className="text-surface dark:text-brand-text font-bold text-title leading-tight">{formatCurrency(depositPaid)}</p>
                <p className="text-surface/60 dark:text-brand-text/60 text-caption mt-0.5">Wapas milega</p>
              </div>
            )}
            {depositPending > 0 && (
              <div className="bg-amber-500/30 dark:bg-amber-500/20 rounded-xl px-3 py-2 border border-amber-500/30">
                <p className="text-amber-100 dark:text-amber-400 text-caption font-bold uppercase">Deposit Baaki ⏳</p>
                <p className="text-surface dark:text-amber-300 font-bold text-title leading-tight">{formatCurrency(depositPending)}</p>
                <p className="text-amber-100 dark:text-amber-400/80 text-caption mt-0.5">Maine nahi diya</p>
              </div>
            )}
            {depositWapas > 0 && (
              <div className="bg-emerald-500/30 dark:bg-emerald-500/20 rounded-xl px-3 py-2 border border-emerald-500/30">
                <p className="text-emerald-100 dark:text-emerald-400 text-caption font-bold uppercase">Wapas Milega 🔄</p>
                <p className="text-surface dark:text-emerald-300 font-bold text-title leading-tight">{formatCurrency(depositWapas)}</p>
              </div>
            )}
          </div>
        </div>
            ...
            return (
              <div key={rental.id} className="bg-surface rounded-2xl border border-border-default shadow-sm overflow-hidden">
                ...
                <div className="p-4 flex justify-between items-start">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <h4 className="font-heading text-title font-bold text-text-primary">{rental.name}</h4>
                      <span className="text-caption font-bold px-2 py-0.5 bg-brand/10 text-brand rounded-full">{rental.type}</span>
                    </div>
                ...
                {(rental.agreementEndDate || rental.agreementNote) && (
                  <div className="mx-4 mb-3 px-3 py-2 bg-blue-500/10 rounded-xl border border-blue-500/20 text-caption text-blue-500 space-y-0.5">
                      ...
                      <div key={payment.id} className="flex justify-between items-center py-2 border-b border-border-subdued last:border-0">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <p className="font-bold text-text-primary text-body-sm">{formatCurrency(payment.amount)}</p>
                            {payment.paidFromDeposit && (
                              <span className="text-caption font-bold px-1.5 py-0.5 bg-brand/10 text-brand rounded-full">Deposit se</span>
                            )}
                          </div>
          ...
          >
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <h3 className="font-heading text-title font-bold text-text-primary">{propEditId ? 'Property Edit' : 'Naya Property'}</h3>
              ...
              <div>
                <label className="text-caption font-bold text-text-subdued uppercase block mb-1.5">Property Name</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-caption font-bold text-text-subdued uppercase block mb-1.5">Owner Name</label>
                ...
                <div>
                  <label className="text-caption font-bold text-text-subdued uppercase block mb-1.5">Owner Phone</label>
          ...
          >
            <div className="p-6 space-y-4">
              ...
              {payForm.isDepositMode && (
                <div className="bg-brand/10 rounded-xl px-3 py-2 text-body-sm text-brand font-bold">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-caption font-bold text-text-subdued uppercase block mb-1.5">Mahina</label>
              ...
              {!payForm.isDepositMode && (
                <button
                  ...
                  className={cn('w-full py-2.5 rounded-xl text-body-sm font-bold border transition-all',
